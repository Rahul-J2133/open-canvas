...
#     cv2.putText(img, "Reset", (20, 45), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
#     return img


def main():
    drawing = False  # Flag to indicate whether drawing should occur
    canvas = np.zeros((480, 640, 3), dtype=np.uint8)  # Create a canvas to draw lines
    ...
    while True:
        ...
        # Draw the canvas on the main frame
        ...
        if hands:
            ...
            if len(lmList) != 0:
                tip_index_x, tip_index_y = lmList[8][1], lmList[8][2]
                print(f"Tip of index finger - X: {tip_index_x}, Y: {tip_index_y}")
            ...
            if length <= 30:
                ...
                if drawing:
                    # Save the drawn lines if drawing has occurred
                    drawing = False
                    prev_point = None

        # img = draw_reset_button(img)
